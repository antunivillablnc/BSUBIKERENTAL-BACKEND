...
router.get('/rental-history', async (_req, res) => {
  try {
    ...
    const rentals = await Promise.all(rentalsSnap.docs.map(async d => {
      ...
      const appData: any = appDoc && appDoc.exists ? appDoc.data() : null;
      const college = (appData?.college ?? appData?.department ?? undefined) ?? (userDoc.exists ? (userDoc.data() as any)?.college : undefined) ?? (r as any)?.college ?? null;
      const applicationOut = appDoc && appDoc.exists ? { id: appDoc.id, ...appData, college: (appData?.college ?? appData?.department ?? null) } : null;
    ...
    const mappedRejections = await Promise.all(rejSnap.docs.map(async aDoc => {
      ...
      const college = (a as any)?.college ?? (a as any)?.department ?? (userDoc.exists ? (userDoc.data() as any)?.college : undefined) ?? null;
