...
router.get('/rental-history', async (_req, res) => {
  try {
    ...
    const mappedRejections = await Promise.all(rejSnap.docs.map(async aDoc => {
      ...
      return { id: `rej-${a.id}`, startDate: null, endDate: null, createdAt: createdIso, user: userDoc.exists ? { id: userDoc.id, ...userDoc.data() } : null, bike: null, application: { id: a.id, firstName: a.firstName, lastName: a.lastName, email: a.email }, type: 'rejected', status: 'Rejected' };
    }));
    ...
    const mappedInProgress = await Promise.all(progSnap.docs.map(async aDoc => {
      ...
      return { id: `app-${a.id}-${a.status}`, startDate: assignedIso || null, endDate: null, createdAt: assignedIso || createdIso, user: userDoc.exists ? { id: userDoc.id, ...userDoc.data() } : null, bike: bikeDoc && bikeDoc.exists ? { id: bikeDoc.id, ...bikeDoc.data() } : null, application: { id: a.id, firstName: a.firstName, lastName: a.lastName, email: a.email }, type: 'rental', status: a.bikeId ? 'Rented' : 'Approved' };
    }));
